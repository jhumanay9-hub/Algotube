<?php
// Handle the Preflight (OPTIONS) request FIRST - MUST be before any other headers
...
// Universal CORS Headers for actual requests
header("Access-Control-Allow-Origin: http://localhost:9002");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $checkStmt = $pdo->prepare("SELECT id FROM subscriptions WHERE follower_id = ? AND creator_id = ? LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
        ...
        if ($followerId && $creatorId) {
            $stmt = $pdo->prepare("SELECT id FROM subscriptions WHERE follower_id = ? AND creator_id = ? LIMIT 1");
