...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($user_id && $video_id) {
        try {
            $stmt = $pdo->prepare("SELECT id FROM dislikes WHERE user_id = ? AND video_id = ? LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $checkStmt = $pdo->prepare("SELECT id FROM dislikes WHERE user_id = ? AND video_id = ? LIMIT 1");
