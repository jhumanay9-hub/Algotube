...
export function SubscriptionProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshSubscriptions = useCallback(async (userId: string) => {
    try {
      const res = await fetch(getApiUrl("subscribe.php", { user_id: userId }));
      ...
      console.error("[SubscriptionContext] Failed to refresh subscriptions:", e);
    }
  }, []);

  // Check if user is subscribed to a creator
  const isSubscribed = useCallback((creatorId: number) => {
    return subscribedCreators.has(creatorId);
  }, [subscribedCreators]);

  // Toggle subscription (subscribe/unsubscribe)
  const toggleSubscription = useCallback(async (creatorId: number): Promise<boolean> => {
    try {
      ...
      console.error("[SubscriptionContext] Failed to toggle subscription:", e);
      return false;
    }
  }, []);
