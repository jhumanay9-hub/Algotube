...
// ============================================================
// Menu Cache (Hash Map) — O(1) lookup for cached sections
// Prevents redundant fetches when sidebar is re-mounted
// ============================================================
...
const Sidebar = React.memo(({ isMobile }: SidebarProps) => {
  ...
  // Fetch subscribed channels with hash map caching
  const fetchSubscribedChannels = useCallback(async () => {
    ...
    // Hash Map Cache Check — O(1) lookup
    const cacheKey = `channels_${Array.from(subscribedCreators).sort().join(",")}`;
    if (menuCache.has(cacheKey)) {
      setSubscribedChannels(menuCache.get(cacheKey)!);
      return;
    }
    ...
    try {
      ...
      // Store in cache for future O(1) retrieval
      menuCache.set(cacheKey, uniqueChannels);
  ...
  // Memoized SidebarItem to prevent re-creation on every render
  ...
  return (
    <aside
      className={cn(
        "glass-panel m-4 mt-0 rounded-2xl p-4 flex flex-col gap-8 overflow-y-auto",
