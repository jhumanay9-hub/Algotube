...
export default function Navbar() {
  ...
  const user = {
    ...
    avatar_url: getMediaUrl("/avatars/default.png"),
  };
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        console.error("XAMPP Sync Error in Navbar:", e);
        // Empty State Handling: Set empty array so UI doesn't crash
        setAllVideos([]);
      }
    }
    load();
    ...
    <nav className="sticky top-0 z-50 glass-panel border-b-0 m-4 rounded-2xl px-6 py-3 flex items-center justify-between">
      <div className="flex items-center gap-4">
        <Sheet>
          ...
          <SheetContent
            side="left"
            className="p-0 border-none bg-transparent w-72"
          >
