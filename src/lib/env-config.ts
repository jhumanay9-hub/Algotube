...
// IMPORTANT: Must match the actual folder name under C:\xampp\htdocs\
export const API_BASE = isLocalhost
  ? "http://127.0.0.1/Algotube-main/Algotube-main/api"
  : "http://algotube.gt.tc/api";

// Image/Uploads Base URL - Points to appropriate uploads directory
export const IMAGE_BASE = isLocalhost
  ? "http://127.0.0.1/Algotube-main/Algotube-main/uploads"
...
export function getMediaUrl(path: string | null | undefined | Blob): string {
  ...
  // Fix legacy DB paths: /Algotube/uploads/ -> Algotube-main/Algotube-main/uploads/
  if (
    cleanPath.startsWith("Algotube/uploads/") ||
    cleanPath.startsWith("Algotube/uploads")
  ) {
    ...
    return `${isLocalhost ? "http://127.0.0.1" : "http://algotube.gt.tc"}/${cleanPath}`;
  }
