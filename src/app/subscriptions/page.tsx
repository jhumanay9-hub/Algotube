  ...
  useEffect(() => {
    async function syncData() {
      ...
      try {
        ...
        // Logging: See exactly what keys the MySQL database is sending
        console.log("Raw Data from PHP (Subscriptions):", data);
        console.log(
          "Data type:",
          typeof data,
          "Is Array:",
        ...
        // Map PHP schema to expected VideoCard schema with defensive mapping
        const mappedData = Array.isArray(data)
          ? data.map((v) => ({
              ...v,
              url: v.file_path || v.url || "", // Defensive: fallback to empty string
              author_name: v.author_name || v.creator || "Local Upload",
              description:
                v.description ||
                `Uploaded on ${v.created_at || "unknown date"}`,
              thumbnail: v.thumbnail_path || v.thumbnail || "",
              views: v.views || 0,
              likes: v.likes || 0,
        ...
        console.error("MySQL Sync failed (Subscriptions):", e);
        ...
        <main className="flex-1 overflow-y-auto p-4 pt-0 custom-scrollbar">
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
              ...
              <p className="font-code text-xs tracking-widest text-accent uppercase">
                Resolving Local MySQL...
              </p>
            </div>
          ) : (
              ...
              {subscribedVideos.length > 0 ? (
                ...
                <div className="col-span-full p-12 glass-panel rounded-3xl text-center border-dashed border-white/5 opacity-50">
                  <p className="text-muted-foreground font-body">
                    No recent transmissions from your linked creators.
                  </p>
                </div>
              )}
            </div>
