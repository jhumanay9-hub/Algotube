...
export default function VideoPageClient() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // ============================================================
      // PARALLEL FETCHING: Video details + Engagement states simultaneously
      // Reduces total load time from 3x sequential → 1x parallel
      // ============================================================
      const fetches = [
        // 1. Video details (primary data)
        fetch(videoDetailUrl, {
          ...
          if (!r.ok) {
            ...
            throw new Error(
              `HTTP ${r.status}: ${err?.error || err?.message || r.statusText}`,
            );
          }
          return r.json();
        }),

        // 2. Engagement states (optional — non-blocking)
        user && user.uid
          ? Promise.allSettled([
              fetch(
              ...
              const [l, d, f] = results.map((r) =>
                r.status === "fulfilled" ? r.value : { active: false },
              );
              return {
                likes: l.active,
                dislikes: d.active,
      ...
      const [videoData, engagement] = await Promise.all(fetches);

      console.log("[VideoPageClient] Raw API Response:", videoData);
      console.log("[VideoPageClient] Engagement states:", engagement);

      // DATA SANITIZATION: Validate and normalize video URL
      let videoUrl = (videoData.url || videoData.file_path || "")
        .toString()
        .trim();
      ...
      // Convert relative DB path to full URL
      if (!videoUrl.startsWith("http")) {
        videoUrl = getMediaUrl(videoUrl);
      }
      videoData.url = videoUrl;

      console.log("[VideoPageClient] Processed Video Data:", videoData);
      setVideo(videoData || null);
      setRecommendations([]);

      // Apply engagement states
      setIsLiked(engagement.likes === true);
      setIsDisliked(engagement.dislikes === true);
      setIsFavorited(engagement.favorites === true);
    } catch (e: any) {
