...
import { ThumbsUp, ThumbsDown, Star, Eye, Sparkles, Loader2, Users, DatabaseZap, Zap, Send, Shield } from 'lucide-react';
...
function VideoContent() {
  ...
  const [externalSyncState, setExternalSyncState] = useState<{ currentTime: number; isPaused: boolean } | undefined>(undefined);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const vRes = await fetch(`/api/video_detail.php?id=${vidStr}`);
      ...
      const recRes = await fetch(`/api/get_feed.php`);
      if (recRes.ok) {
        const vData = await recRes.json();
        const vids = Array.isArray(vData) ? vData.map((v: any) => ({
          ...v, 
  ...
  const handleInitiateParty = async () => {
    ...
    try {
      const res = await fetch('/api/initiate_party.php', {
        method: 'POST',
      ...
      if (data.status === 'success') {
        ...
        toast({ title: "Party Initialized", description: `Room Code: ${data.room_code}. Broadcasting to mesh...` });
      }
    } catch (e) {
      toast({ variant: "destructive", title: "Mesh Error", description: "Could not initiate watch party." });
    }
  };

  const syncHostState = useCallback(async (overrides?: Partial<{currentTime: number, isPaused: boolean}>) => {
    if (!isHost || !activeParty || !user) return;
    
    const state = {
      room_code: activeParty,
      host_id: user.uid,
      current_time: overrides?.currentTime ?? playerRef.current?.getCurrentTime() ?? 0,
      is_paused: overrides?.isPaused ?? playerRef.current?.getIsPaused() ?? true
    };

    try {
      await fetch('/api/sync_party.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleInteraction = async (type: 'likes' | 'dislikes' | 'favorites') => {
    ...
    if (type === 'likes') {
      ...
      setVideo((prev: any) => ({
        ...prev,
        likes: nextLiked ? (prev.likes || 0) + 1 : Math.max(0, (prev.likes || 0) - 1),
        dislikes: prevDisliked && nextLiked ? Math.max(0, (prev.dislikes || 0) - 1) : prev.dislikes
      }));
      ...
      setVideo((prev: any) => ({
        ...prev,
        dislikes: nextDisliked ? (prev.dislikes || 0) + 1 : Math.max(0, (prev.dislikes || 0) - 1),
        likes: prevLiked && nextDisliked ? Math.max(0, (prev.likes || 0) - 1) : prev.likes
      }));
    ...
    try {
      const res = await fetch(`/api/interact.php`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      toast({ variant: "destructive", title: "Sync Failed", description: "Interaction could not be registered." });
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col h-screen items-center justify-center gap-4 bg-background">
        ...
        <p className="font-code text-sm tracking-widest text-emerald-500 uppercase animate-pulse">Querying Registry...</p>
      </div>
    );
  }
          ...
          <div className="flex-1 flex flex-col gap-6">
            ...
            <div className="glass-panel rounded-3xl p-8 relative overflow-hidden bg-zinc-950/50 border border-emerald-500/10">
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-4">
                  <span className="bg-emerald-500/20 text-emerald-400 text-[9px] font-code px-2 py-0.5 rounded-full border border-emerald-500/30 uppercase">SQL Sync Mesh</span>
                </div>

                <h1 className="text-3xl font-headline font-bold mb-6 text-white">{video?.title}</h1>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-headline font-bold text-lg text-white">{video?.author_name || "Mesh Creator"}</p>
                  ...
                  <div className="flex items-center gap-2">
                    <Button 
                      ...
                      className={cn(
                        ...
                        isLiked ? "text-emerald-500 bg-emerald-500/10 border border-emerald-500/30 shadow-[0_0_10px_rgba(16,185,129,0.2)]" : "bg-white/5 border border-white/10 hover:bg-white/10"
                      )}
                    >
                      <ThumbsUp size={16} className={isLiked ? "fill-emerald-500" : ""} /> 
                      <span className="text-[10px] font-bold">{video?.likes || 0}</span>
                    </Button>
                    <Button 
                      variant="ghost" 
                    ...
                    >
                      ...
                      <span className="text-[10px] font-bold">{video?.dislikes || 0}</span>
                    </Button>
                    <Button 
                      variant="ghost" 
                    ...
                    {!activeParty && (
                      ...
                      >
                        ...
                        <span className="font-bold text-[10px] uppercase tracking-widest">Start Party</span>
                      </Button>
                    )}
                    <Button 
                      variant="ghost" 
                      onClick={() => setShowConversation(!showConversation)}
                      className={cn("rounded-2xl gap-2 h-10 px-4 border transition-all duration-300", showConversation ? "border-emerald-500 text-emerald-500 bg-emerald-500/5" : "bg-white/5 border border-white/10")}
                    >
                      <Users size={16} /> 
                ...
                <div className="bg-black/40 rounded-2xl p-6 border border-white/5">
                  ...
                  <p className="text-zinc-300 leading-relaxed font-body text-sm">{video?.description || "No encrypted metadata provided for this transmission."}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mb-12">
              <div className="flex items-center gap-3 mb-8">
                ...
                <h3 className="text-lg font-headline font-bold uppercase tracking-widest text-white">Mesh Affinity</h3>
              </div>
...
export default function VideoDetailPage() {
  return (
    <Suspense fallback={
      <div className="flex flex-col h-screen items-center justify-center gap-4 bg-background">
        <Loader2 className="animate-spin text-emerald-500" size={48} />
        <p className="font-code text-sm tracking-widest text-emerald-500 uppercase animate-pulse">Initializing Mesh...</p>
      </div>
    }>
      <VideoContent />
    </Suspense>
